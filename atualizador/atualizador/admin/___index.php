...
$query_participantes = "SELECT participantes.cod_participante, participantes.numero_pessoa, participantes.cpf, participantes.nome_cliente, contascorrentes.contacorrente FROM participantes, contascorrentes WHERE contascorrentes.participante=participantes.numero_pessoa ORDER BY contascorrentes.contacorrente";
$participantes = mysql_query($query_participantes, $conecta) or die(mysql_error());
...
do {  
?>
                    <option value="<?php echo $row_participantes['cod_participante']?>"<?php if (!(strcmp($row_participantes['cod_participante'], $_GET['participante']))) {echo "selected=\"selected\"";} ?>><?php echo $row_participantes['contacorrente']?> - <?php echo $row_participantes['nome_cliente']; ?></option>
                    <?php
...
<input name="n_cupons" type="text" id="n_cupons" size="5" maxlength="3" disabled="disabled" />
<br />
<input type="button" name="Button" value="Gerar" onclick="document.form1.action='geracupons.php';document.form1.submit();" />
<input name="delete" type="button" id="delete" value="Remover Cupons" onclick="document.form1.action='delete.php';document.form1.submit();" />
</p>
                <p>
                  <input name="report" type="button" id="report" value="Relat&oacute;rios" onclick="location.href='relatorio.php';" />
                </p>
            </form>   
                <iframe align="top" frameborder="0" height="200" id="listacupons" name="listacupons" scrolling="auto" width="650" src="">listacupons.php</iframe></td>
          </tr>
        </table>
        <p></p>
