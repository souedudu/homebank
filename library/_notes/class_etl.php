...
class ETL{
	...
	*/
	...
	// Construtor da Classe ETL, executa verificões básicas.
		...
		if(is_array($ArrayConf))
		{
			if(is_array($Detalhe))
			{
				...
				{ // Valida diretório
					...
					}else{$this->Mensagem("Não foi possível abrir o arquivo.");}
					...
					if($this->ArrayDetalheTotal > 0)
					{
						...
						{							 							
							...
							}elseif($this->ArrayDetalhe[$w]['tipo']=='d')
							{
								...
								{
								  	//###########################################
								  	//				FAZ INVERSÂO DATA AQUI
								  	//###########################################

									$resDados.=$this->TrataData($registro);
									//$resDados.="'".$registro."'";																									

									//echo "resDados: ".$resDados."<BR>";
						...
						if ($this->DbTabela == "cliente")
						{
						  ...
						  //echo "sqlupdate: ".$sqlupdate."<BR><BR>";
						...
						//echo "QueryRefinada: ".$QueryRefinada."<br/>";
                        if ($endercocli == "0")
                        {
                            $endercocli = '';
