  ...
  if ($bttipoacao == "Incluir")
  {
	...
	if ($_SESSION['codcliente'] != "")
	{
       $sql = "insert into solicitacaoserv (codcliente, codtiposervsol, numcontacorrente, dessolicitacao, desemailcont, numtelefonecont, dtsolicitacao, hrsolicitacao) values (".$_SESSION['codcliente'].",".$codtiposervsol.",'".$numcontacorrentesol."','".$dessolicitacao."','".$desemailcont."','".$numtelefonecont."','".$dtsolicitacao."', '".$hora."')";
	      	 ...
	      	 $onhref = "solicitacao.php?tipoacao=Incluir";
	}
...
//			."', desemailcont = '".$desemailcont."', numtelefonecont = '".$numtelefonecont."'". 
	...
	$onhref = "javascript://;";
	$onclick = "<script>alert('Triagem efetuada com sucesso.');window.opener.location.reload();window.close();</script>";
	echo $onclick;
  }
  ...
  if ($bttipoacao == "Incluir"){
	...
	//ENVIO DE EMAIL PARA CLIENTE E PARA O SAC INFORMANDO O REGISTRO DE OS
	...
	*/
	$target = "sac@crediembrapa.com.br; ".$desemailcont;
	
	/*
	//  ASSUNTO DO EMAIL
	*/
	if ($_REQUEST['tipoacao'] == "Incluir"){
		$assuntoMsg = "Cadastro da ";
	}else{
		$assuntoMsg = "Alteração da ";	
	}
	
	
	$subject = "[CrediEmbrapa - HomeBanking] - ".$assuntoMsg." OS ".$codigosolicitacao.".";
	...
	//SQL para buscar dados do Tipo de Solicitação feita pelo Cliente, de acordo com o nº da Solicitação
	 $sqlString = "Select t.* From tiposervsolicitacao t where t.codtiposervsol = ".$codtiposervsol;	       	 
	 ...
	 $sqlString = "Select te.* From tecnicoresp te where te.codtecnicoresp = ".$_SESSION['codtecnicorespadm'];	       	 
     $rsqry = mysql_query($sqlString);
     $dadostecnicoresp = mysql_fetch_array($rsqry);
  	...
  	$msg = "<table width='570' cellspacing='0' border='0'>
				<tr>
					<td align='justify'>
						<p align='center'><b>Informação de Cadastro de OS</b></p><br>
							Informamos que foi Cadastrada a seguinte OS: 						
					</td>
				</tr>
				<tr>
					<td align='justify'>
						<strong>Código da OS:</strong> ".$codigosolicitacao."<br>"; 
						if ($_SESSION['codtecnicorespadm'] != ""){ 
							$msg .= "<strong>OS Triada para o Técnico:</strong> ".$dadostecnicoresp['nomtecnicoresp']."<br>";
						}
						$msg .= "<strong>Data de Abertura da OS:</strong> ".FormataData($dtsolicitacao,'pt')."<br>											
						 		 <strong>Tipo de OS:</strong> ".$dadostiposervsol['destiposervsol']."<br>
						 	 	 <strong>Conta Corrente do Cliente:</strong> ".$numcontacorrentesol."<br>
						 		 <strong>Descrição da OS:</strong> ".$dessolicitacao."<br>						 
						 		 <strong>Nº de Telefone de Contato:</strong> ".$numtelefonecont."<br>
						 		 <strong>E-mail de Contato:</strong> ".$desemailcont."<br>						 
					</td>
				</tr>
			</table>						
          ";
  ...
  $html = "<HTML><BODY><table width='570' border='0' cellspacing='0'>
  						<tr>
      						<td>
		    ...
		    <font color=black>$msg</font></BODY></HTML>";
