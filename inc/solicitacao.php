...
<?
	...
	*/
	...
	//  ASSUNTO DO EMAIL
	*/
	$subject = "[CrediEmbrapa - HomeBanking] - CADASTRO DE OS.";
	...
	//SQL para buscar dados do Tipo de Solicitação feita pelo Cliente, de acordo com o nº da Solicitação
	 ...
	 $sqlString = "Select te.* From tecnicoresp te where te.codtecnicoresp = ".$codtecnicosol;	       	 
     $rsqry = mysql_query($sqlString);
     $dadostecnicoresp = mysql_fetch_array($rsqry);
  	...
  	$msg = "<table width='100%' cellspacing='0' border='0'>
				...
				<tr>
					<td align='justify'>"; 
					if ($codtecnicosol != ""){ 
						$msg .= "<strong>Nome Técnico Soliticitante:</strong> ".$dadostecnicoresp['nomtecnicoresp']."<br>";
					}
				$msg .= "<strong>Data de Abertura da OS:</strong> ".FormataData($dtsolicitacao,'pt')."<br>
						 <strong>Horário de Abertura da OS:</strong> $hora<br>
						 <strong>Tipo de OS:</strong> ".$dadostiposervsol['destiposervsol']."<br>
						 <strong>Conta Corrente do Solicitante:</strong> $numcontacorrentesol<br>
